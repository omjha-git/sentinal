...
const require_utils = require('./utils-BTLHuXdZ.js');
...
const authenticateRequest = (opts) => {
	...
	const clerkRequest = (0, _clerk_backend_internal.createClerkRequest)(require_utils.incomingMessageToRequest(request));
	const env = {
		...require_utils.loadApiEnv(),
		...require_utils.loadClientEnv()
	};
	...
	const middleware = async (request, response, next) => {
		...
		if (frontendApiProxy) {
			...
			if ((typeof frontendApiProxy.enabled === "function" ? frontendApiProxy.enabled(requestUrl) : frontendApiProxy.enabled) && (requestUrl.pathname === proxyPath || requestUrl.pathname.startsWith(proxyPath + "/"))) {
				const proxyResponse = await (0, _clerk_backend_proxy.clerkFrontendApiProxy)(require_utils.requestToProxyRequest(request), {
					proxyPath,
					publishableKey,
					secretKey
				});
